...
<guid isPermaLink="true"><?= generatePostURL($post, $postBaseUrl); ?></guid>
...
if (!empty($post->image_url)) {
    ...
    if (!empty($imagePath) && file_exists(FCPATH . $imagePath)) {
        $fileSize = @filesize(FCPATH . $imagePath) ?: 49398;
    }
}
if (!empty($imageUrl)) {
    $ext = strtolower(pathinfo(parse_url($imageUrl, PHP_URL_PATH) ?? '', PATHINFO_EXTENSION));
    $mimeMap = ['webp' => 'image/webp', 'png' => 'image/png', 'gif' => 'image/gif', 'svg' => 'image/svg+xml', 'avif' => 'image/avif'];
    $imageMime = $mimeMap[$ext] ?? 'image/jpeg';
?>
<enclosure url="<?= $imageUrl; ?>" length="<?= $fileSize; ?>" type="<?= $imageMime; ?>"/>
<?php } ?>
