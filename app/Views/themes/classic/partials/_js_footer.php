...
<script>let mouseMoveCount = 0, scrollCount = 0, debounceTimer, mouseMoveDebounceTimer, debounceDelay = 8;var pageReferrerCaptured = (document.referrer || '');function incrementMouseMove() {clearTimeout(mouseMoveDebounceTimer);mouseMoveDebounceTimer = setTimeout(function () {mouseMoveCount++;}, debounceDelay);}document.addEventListener('mousemove', incrementMouseMove);document.addEventListener('touchmove', incrementMouseMove);window.addEventListener('scroll', function () {clearTimeout(debounceTimer);debounceTimer = setTimeout(function () {scrollCount++;}, debounceDelay);});setTimeout(function () {let userBehaviorData = {postId: '<?= $postJsonLD->id; ?>', mouseMoveCount: mouseMoveCount, scrollCount: scrollCount, pageReferrer: pageReferrerCaptured, pageUrl: (window.location && window.location.href) || ''};$.ajax({type: 'POST', url: VrConfig.baseURL + '/Ajax/incrementPostViews', data: setAjaxData(userBehaviorData)});}, <?= $postTimeSpent; ?>);</script>
<?php endif; ?>
