<?php $pageTitle = 'Meus cursos'; echo view('courses/_head', ['pageTitle' => $pageTitle, 'totalXp' => $totalXp]); ?>
    ...
    <div style="flex:1;min-width:220px">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap">
            <?php foreach ($achievements as $a): ?>
                <span class="jn-tag" style="padding:6px 10px;border-color:var(--gx-gold);color:var(--gx-gold)" title="<?= esc($a['description'] ?? '', 'attr') ?>">🏅 <?= esc($a['name']) ?></span>
            <?php endforeach; ?>
        </div>
    </div>
    <?php endif; ?>
...
<?php else: ?>
    <div class="ac-row"><div class="ac-track">
        <?php foreach ($courses as $c): ?>
            <?= view('courses/_card', ['c' => $c, 'enroll' => $c['_enroll'] ?? null]) ?>
        <?php endforeach; ?>
    </div></div>
<?php endif; ?>
