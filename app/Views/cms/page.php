                ...
                if (!function_exists('cms_render_block')) {
                    ...
                    function cms_render_block($b){
                        ...
                        if ($t=='heading') {
                            ...
                            $items = $b['items'] ?? [];
                            $autoplay = !empty($b['autoplay']);
                            $dots = !empty($b['dots']);
                            $pause = !empty($b['pause']);
                            $interval = (int)($b['interval'] ?? 4000);
                            if (is_array($items) && count($items)) {
                                ...
                                echo '<div class="wm-nav" style="margin-top:6px; text-align:center;"><button class="btn btn-xs btn-default wm-prev">◀</button> <button class="btn btn-xs btn-default wm-next">▶</button>'; 
                                if ($dots) { echo ' <span class="wm-dots"></span>'; }
                                echo '</div>';
                                echo '</div></div>';
                            }
                        } elseif ($t=='accordion') {
                            $items = $b['items'] ?? [];
                            $single = !empty($b['single_open']);
                            if (is_array($items) && count($items)) {
                                echo '<div class="'.$cls.'" style="'.$styleStr.'"><div class="wm-accordion'.($single?' wm-acc-single':'').'">';
                                foreach ($items as $ix=>$it) {
                                    $ti = esc($it['title'] ?? ('Item '.($ix+1)));
                                    $co = esc($it['content'] ?? '');
                            ...
                            $cols = $b['cols'] ?? [[],[]];
                            $layout = $b['layout'] ?? [];
                            $count = is_array($cols) ? count($cols) : 2;
                            // compute widths
                            $widths = [];
