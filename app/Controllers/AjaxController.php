...
class AjaxController extends BaseController
{
    ...
    public function addSimulatorLeadPost()
    {
        ...
        try {
            ...
            log_message('info', 'Requisição ao addSimulatorLeadPost recebida');
            
            $name = cleanStr($this->request->getPost('name'));
            ...
            if (empty($name)) {
                $jsonData['message'] = "Nome é obrigatório";
                echo json_encode($jsonData);
                exit();
            }
            
            if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
                $jsonData['message'] = "Email inválido";
                echo json_encode($jsonData);
                exit();
            }
            
            if (empty($phone)) {
                $jsonData['message'] = "Telefone é obrigatório";
                echo json_encode($jsonData);
                exit();
            }
